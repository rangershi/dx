...
const VERCEL_DOWNLOADED_ENV_PATH = '.vercel/.env.production.local'

function getTargetConfig(target, configuredTarget) {
  if (configuredTarget && typeof configuredTarget === 'object') {
    return {
      configFile: configuredTarget.configFile,
      projectIdEnvVar: configuredTarget.projectIdEnvVar,
      deployCwd: configuredTarget.deployCwd ?? '.',
      deployMode: configuredTarget.deployMode ?? 'prebuilt',
      prebuiltCwd: configuredTarget.prebuiltCwd ?? configuredTarget.deployCwd ?? '.',
    }
  }
...
function clearDownloadedVercelEnv(contextRoot) {
  rmSync(join(contextRoot, VERCEL_DOWNLOADED_ENV_PATH), { force: true })
}
...
export async function deployToVercel(target, options = {}) {
  const {
    ...
    strictContext = true,
    run = runVercel,
    targetConfig: configuredTarget = null,
  } = options
  ...
  for (const t of targets) {
    const resolvedTargetConfig = getTargetConfig(
      t,
      targets.length === 1 ? configuredTarget : null,
    )
    if (!resolvedTargetConfig) {
      logger.error(`不支持的部署目标: ${t}`)
      logger.info('请在 commands.json 的 deploy.<target>.vercel 中声明 Vercel target')
      process.exitCode = 1
      return
    }
    if (!resolvedTargetConfig.configFile || !resolvedTargetConfig.projectIdEnvVar) {
      logger.error(`Vercel target 配置不完整: ${t}`)
      logger.info('必须声明 configFile 和 projectIdEnvVar')
      process.exitCode = 1
      return
    }
  }

  const targetConfigs = targets.map(t =>
    getTargetConfig(t, targets.length === 1 ? configuredTarget : null),
  )
  ...
  for (const t of targets) {
    const targetConfig = getTargetConfig(t, targets.length === 1 ? configuredTarget : null)
    ...
    try {
      ...
      clearDownloadedVercelEnv(prebuiltCwd)
    }
  }
}
