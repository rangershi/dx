...
export function resolveEnvProfilePaths({ projectRoot, profile, environment }) {
  ...
  return {
    source: join(projectRoot, `.env.${profile}.${environment}.local`),
    target: join(projectRoot, `.env.${environment}.local`),
    template: join(projectRoot, 'dx', 'env', 'templates', profile, `${environment}.local.example`),
  }
}
...
function recoverStaleTransaction(projectRoot, environment, transaction) {
  ...
  const expectedBackup = `${expectedTarget}.dx-backup`
  if (transaction.target !== expectedTarget || transaction.backup !== expectedBackup) return
  if (
  ...
  if (transaction.hadTarget) {
    if (existsSync(expectedBackup)) {
      rmSync(expectedTarget, { force: true })
      renameSync(expectedBackup, expectedTarget)
    }
  } else {
    rmSync(expectedTarget, { force: true })
    rmSync(expectedBackup, { force: true })
  }
...
function materializeProfile(source, target, onPrepared) {
  ...
  const backup = `${target}.dx-backup`
  const hadTarget = existsSync(target)
  assert(!existsSync(backup), `检测到未恢复的 env 备份: ${backup}`)
  const transaction = { target, temporary, backup, hadTarget }
